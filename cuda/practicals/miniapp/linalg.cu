...
__global__
void axpy(const double* x, double* y, const double alpha, int n){
...
__global__
void scale_diff(double* y, const double* l,  const double* r, const double alpha, int n){
    auto i = threadIdx.x + blockIdx.x*blockDim.x;
    if (i<n){
        y[i] = alpha*(l[i]-r[i]);
    }
...
__global__
void scale(double* y, const double* x, const double alpha, int n){
    auto i = threadIdx.x + blockIdx.x*blockDim.x;
    if (i<n){
        y[i] = alpha*(x[i]);
    }
...
__global__
void lcomb(double* y, double* x, const double* z, const double alpha, const double beta, int n){
    auto i = threadIdx.x + blockIdx.x*blockDim.x;
    if (i<n){
...
void ss_axpy(Field& y, const double alpha, Field const& x)
{
    ...
    kernels::axpy<<<grid_dim, block_dim>>>(x.device_data(),y.device_data(), alpha, n);
}
...
void ss_scaled_diff(Field& y, const double alpha, Field const& l, Field const& r)
{
    ...
    kernels::scale_diff<<<grid_dim, block_dim>>>(y.device_data(), l.device_data(), r.device_data(), alpha, n);
}
